...
export const updatePlan = async (
  ...
  const updated = currentPlan.map((p: any) =>
    ...
    p.place === oldPlan.place
      ? { date: newPlan.date, time: newPlan.time, title: newPlan.title, place: newPlan.place }
      : p
  )
...
export const updateCompany = async (
  companyId: string,
  data: Partial<Pick<company_type, 'name' | 'capital' | 'director' | 'summary' | 'scale' | 'priority'>>
) => {
  const uid = auth.currentUser?.uid
  if (!uid) throw new Error("ログインが必要です")
